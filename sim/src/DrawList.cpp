...
size_t DrawList::addMiniCheetah(Vec4<float> color, bool useOld, bool canHide) {
  ...
  // set model offsets:
  QMatrix4x4 bodyOffset, eye;// upper, lower,
  QMatrix4x4 abadOffsets[4],upperOffsets[4],lowerOffsets[4];;
  eye.setToIdentity();
  ...
  // lower
  lowerOffsets[0].setToIdentity();
  lowerOffsets[0].rotate(90, 1, 0, 0);//
  lowerOffsets[0].rotate(-90, 0, 0, 1);//

  lowerOffsets[1].setToIdentity();
  lowerOffsets[1].rotate(90, 1, 0, 0);
  lowerOffsets[1].rotate(-90, 0, 0, 1);

  lowerOffsets[2].setToIdentity();
  lowerOffsets[2].rotate(90, 1, 0, 0);
  lowerOffsets[2].rotate(-90, 0, 0, 1);

  lowerOffsets[3].setToIdentity();
  lowerOffsets[3].rotate(90, 1, 0, 0);
  lowerOffsets[3].rotate(-90, 0, 0, 1);

  SolidColor bodyColor, abadColor, link1Color, link2Color;
  bodyColor.rgba = useOld ? Vec4<float>(255.0/255.0, 255.0/255.0, 0.0/255.0, .79) : color;
  bodyColor.useSolidColor = true;

  abadColor.rgba = useOld ? Vec4<float>(0.0, 0.0, 0.0, .79) : color;
  abadColor.useSolidColor = true;

  link1Color.rgba = useOld ? Vec4<float>(0.0/255.0, 173.0/255.0, 255.0/255.0, .99) : color;
  link1Color.useSolidColor = true;

  link2Color.rgba = useOld ? Vec4<float>(57.0/255.0, 53.0/255.0, 181.0/255.0, .99) : color;
  link2Color.useSolidColor = true;
  ...
  _nTotal++;
    ...
    for (int i = 0; i < 4; i++) {
        ...
        _objectMap.push_back(i0 + 2);
        _canBeHidden.push_back(canHide);
        _modelOffsets.push_back(upperOffsets[i]);//s[i]);
        _kinematicXform.push_back(eye);
        _instanceColor.push_back(link1Color);
